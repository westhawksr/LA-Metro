...
C-------------------------------------------------------------------
      SUBROUTINE MACCEGR(UNIT1,UNIT2,STASUM,STASUM2)
      INCLUDE 'stadat.com'
      INCLUDE 'param.com'
      INCLUDE 'mtamcpar.inc'
      INTEGER*2 T,K,T1,UNIT1,UNIT2
 ...
 4441 FORMAT('COMMUTER_RAIL_ACCESS_VOLUMES'/
     ...
     *            STASUMP(K,19)
      ...
      IF(STASUMP(K,5).GT.0.1) THEN
	    WRITE(UNIT1,9211) KS,STANAME(K),STASUMP(K,1),STASUMP(K,19),
     *                  (STASUMP(K,L),L=2,5)
 9211    FORMAT(2X,I4,3X,A29,1X,6F8.0)
      WRITE(UNIT2,4440) KS,STANAME(K),STASUMP(K,1),STASUMP(K,19),
     *                  (STASUMP(K,L),L=2,5)
 4440 FORMAT(I4,',',A37,6(',',F8.0))
      END IF
  500 CONTINUE
      WRITE(UNIT1,9337) TTWLK,TTBIK,TTBUS,TTPNR,TTKNR,TTOTAL
 9337 FORMAT(/3X,'TOTAL',31X,6F8.0)
      WRITE(UNIT2,4442) TTWLK,TTBIK,TTBUS,TTPNR,TTKNR,TTOTAL
 ...
 9212 FORMAT(//,30X,'R E P O R T   3A',/,
     ...
     *          '--------','--------','--------','--------','------  ')
      ...
      IF(DENOM.GT.0.1) THEN
	    WRITE(UNIT1,9213) KS,STANAME(K),
     *                  (STASUMA(K,L),L=1,4),DENOM
    	WRITE(UNIT2,4440) KS,STANAME(K),
     *                  (STASUMA(K,L),L=1,4),DENOM
 9213 FORMAT(2X,I4,3X,A29,1X,5F8.0)
      END IF
 1501 CONTINUE
      WRITE(UNIT1,9337) TTWLK,TTBUS,TTBIK,TTPNR,TTOTAL
      WRITE(UNIT2,4442) TTWLK,TTBUS,TTBIK,TTPNR,TTOTAL
C
      WRITE(UNIT1,9310)
 ...
 4444 FORMAT('URBAN_RAIL_ACCESS_VOLUMES'/
     ...
     *             STASUMP(K,20)
      ...
      IF(STASUMP(K,10).GT.0.1) THEN
	    WRITE(UNIT1,9211) KS,STANAME(K),STASUMP(K,6),STASUMP(K,20),
     *               (STASUMP(K,L),L=7,10)
      WRITE(UNIT2,4440) KS,STANAME(K),STASUMP(K,6),STASUMP(K,20),
     *                  (STASUMP(K,L),L=7,10)
      END IF
 1502 CONTINUE
      WRITE(UNIT1,9337) TTWLK,TTBIK,TTBUS,TTPNR,TTKNR,TTOTAL
 ...
 4446 FORMAT('BRT_ACCESS_VOLUMES'/
     ...
     *             +STASUMP(K,18)+STASUMP(K,23)
      ...
      IF(STASUMP(K,24).GT.0.1) THEN
	    WRITE(UNIT1,9211) KS,STANAME(K),STASUMP(K,15),STASUMP(K,23),
     *               (STASUMP(K,L),L=16,18),STASUMP(K,24)
      WRITE(UNIT2,4440) KS,STANAME(K),STASUMP(K,15),STASUMP(K,23),
     *               (STASUMP(K,L),L=16,18),STASUMP(K,24)
      END IF
      END DO
      WRITE(UNIT1,9337) TTWLK,TTBIK,TTBUS,TTPNR,TTKNR,TTOTAL
