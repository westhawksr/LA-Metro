...
C
     ...
     *    (stasum(ik,11).gt.0.0.or.stasum(ik,13).gt.0.0)) go to 500
       K=EQUIV(IK)
       IF(K.EQ.0) GO TO 500
       STASUM(K,25)=STASUM(K,25)+STASUM(IK,3) + STASUM(IK,8) +
     *              STASUM(IK,12)*(BPFACT/AUTOCC) + STASUM(IK,14)*
     *              (BPFACE/AUTOCC) + STASUM(IK,17)
       STASUM(K,26)=STASUM(K,26) + STASUM(IK,5) + STASUM(IK,10) +
     *              STASUM(IK,11) + STASUM(IK,12)+ STASUM(IK,13) +
     *              STASUM(IK,14) + STASUM(IK,24)
	 endif
 500   CONTINUE
C
...
C
      ...
      K=EQUIV(IK)
      IF(K.EQ.0) GO TO 510
      STASUM(IK,25)=STASUM(K,25)
      STASUM(IK,26)=STASUM(K,26)
  510 CONTINUE
C
C STATION TYPE LOOP
C
...
C
      ...
      IF(STADATA(K,7).NE.1) GO TO 100
      IF(SELCAP.AND.STADATA(K,22).NE.1.0) GO TO 100
      ks=MAX_IZONES+k
      VC=0.0
      VC2=0.0
      OBS=0.0
...
C
        ...
        VC=(STASUM(K,25)+(TEMP*2.0))/(STADATA(K,4)*2.0)
        ENDIF
C
C  COMPUTE REMAINING CAPACITY V/C RATIO
C
       IF(stasum(k,25).gt.0.0) then
       OBS=(STADATA(K,3)*2.0)/STASUM(K,25)
...
C
	if(stadata(k,6).eq.1.0) then
       ...
       SPACES=STASUM(K,25)/2.0
	WRITE(26,101) KS,STANAME(K),SPACES,STADATA(K,4),TEMP,VC,
...
C..SHADOW PRICE
       ...
       IF(STADATA(NS,5).GT.0) WRITE(11,8333) NSTA,STADATA(NS,5)
 8333  FORMAT(2x,i4,2x,f6.0)
C..SPACES USED
       USED=(STASUM(NS,25)/2.0)+(STADATA(NS,4)-STADATA(NS,3))
       IUSED=IFIX(USED)
       IF(SELCAP.AND.STADATA(NS,22).EQ.1.0) THEN
       IF(IUSED.GT.0) WRITE(9,8335) NSTA,IUSED
 8335  FORMAT(2X,I4,2X,I5)
       ENDIF
       IF((.NOT.SELCAP).AND.IUSED.GT.0) WRITE(9,8335) NSTA,IUSED
       ENDIF
  411  CONTINUE
