...
C     INTEGER*2     IHR,IMIN,ISEC,I100
      ...
      INTEGER*2     ITEMP1,URIND(14),UNIT1,MAXDIST
...
C
      ...
      REAL*8        ETRNLSUM(7)
C
      REAL*8        TRIPS(74,MAX_ZONES),LSHARE
      ...
      REAL*8        LSAUTO,STASUM(MAX_STATIONS,26),TTWLK,TTDRV,TTOTAL
...
C
     ...
     *            'KNR_CRBR','BIKE_CRBR','TOT_CRBR'/
      ...
      DATA SNAME/'Urban_Rail','Blended','Total'/
C----------------------------------------------------------------------
C
      FF=CHAR(12)
C               
...
C
     ...
     *  CALL STATION(STASTA,IMODE,INDAPM,NHBACC,NHBUTL)
      ...
      CALL STASEL(SELIND,NAME,ZSTASTA)
      WRITE(*,9900)
 9900 FORMAT(/)
      IF(NHBDIR) THEN
...
c...commuter rail
     ...
     *                  WDSTA,BIKDSTA,BDSTA,PDSTA,KDSTA,LSHARE)
      IF(RAILSEL) THEN
      CALL SELSUM(IMODE,IZ,JZ,OSTA,ASTA,TCRT,STATRIPS,BLDTRIPS,
     *                  CHWIND,CHBKIND,CHBIND,CHPIND,CHKIND,
...
c...urban rail
     ...
     *                  WDSTA,BIKDSTA,BDSTA,PDSTA,KDSTA,LSHARE)
      IF(RAILSEL) THEN
      CALL SELSUM(IMODE,IZ,JZ,OSTA,ASTA,TURT,STATRIPS,BLDTRIPS,
     *                  CHWIND,CHBKIND,CHBIND,CHPIND,CHKIND,
     *                  WDSTA,BIKDSTA,BDSTA,PDSTA,KDSTA,
     *                  SELIND,SELTRIPS,SELDIST,ZSTATRIPS,ZSTATRIPS2,
     *                  BLDURTRIPS,ZSTASTA)
...
c...bus rapid transit
     ...
     *                  WDSTA,BIKDSTA,BDSTA,PDSTA,KDSTA,LSHARE)
      IF(RAILSEL) THEN
      CALL SELSUM(IMODE,IZ,JZ,OSTA,ASTA,TBRTT,STATRIPS,BLDTRIPS,
     *                  CHWIND,CHBKIND,CHBIND,CHPIND,CHKIND,
     *                  WDSTA,BIKDSTA,BDSTA,PDSTA,KDSTA,
     *                  SELIND,SELTRIPS,SELDIST,ZSTATRIPS,ZSTATRIPS2,
     *                  BLDURTRIPS,ZSTASTA)
...
c.....total
      ...
      rem124=0.0
      do jj=1,max_zones
      if(zstatrips(jj).gt.0.0.or.zstatrips2(jj).gt.0.0) then
      temp=((zstatrips(jj)+zstatrips2(jj))*100.0)+rem124
      row(jj)=ifix(temp)
      rem124=temp-row(jj)
      else
      row(jj)=0
...
c.....blended station-to-station trips 
      row=0.0
      purp=2
      if(ii.gt.max_izones) then
      do ij=1,max_stations
	    jj=ij+max_izones
	    in=ii-max_izones
	    if(bldtrips(in,ij).gt.0.0) then
      temp=(bldtrips(in,ij)*100.0)+rem126
      row(jj)=ifix(temp)
      rem126=temp-row(jj)
      else
      row(jj)=0
      ...
      CALL OUTAB(124,ROW,II,PURP,DUMMY,IO)   
c.....blended urban rail station-to-station trips
      row=0.0
      purp=3
      if(ii.gt.max_izones) then
      do ij=1,max_stations
	    jj=ij+max_izones
	    in=ii-max_izones
	    if(bldurtrips(in,ij).gt.0.0) then
      temp=(bldurtrips(in,ij)*100.0)+rem127
      row(jj)=ifix(temp)
      rem127=temp-row(jj)
      else
      row(jj)=0
...
C
      CALL MACCEGR(26,46,STASUM,STASUM2)
      IF(MODEAERPT) THEN
      CALIB=.TRUE.
      CALL MACCEGR(139,140,STASUM,STASUM2)
      CALIB=.FALSE.
      END IF
      IF(CALIB.OR.MODEAERPT) THEN
      IF(CALIB) UNIT1=26
      IF(MODEAERPT) UNIT1=139
      WRITE(UNIT1,9210)
 9210 FORMAT(//,30X,'R E P O R T   3A',/,
     ...
     *            STASUM(K,19)
	 ...
	 WRITE(UNIT1,9211) KS,STANAME(K),STASUM(K,1),STASUM(K,19),
     *                  (STASUM(K,L),L=2,5)
...
C
	...
	WRITE(UNIT1,9211) KS,STANAME(K),STASUM(K,6),STASUM(K,20),
     *               (STASUM(K,L),L=7,10)
      END IF
 1502 CONTINUE
      WRITE(UNIT1,9337) TTWLK,TTBIK,TTBUS,TTPNR,TTKNR,TTOTAL
...
C
	...
	WRITE(UNIT1,9211) KS,STANAME(K),STASUM(K,15),STASUM(K,23),
...
C
     ...
     *             STASUM4(K,5)
        ...
        TTBIK=TTBIK+STASUM4(K,5)
	      ...
	      TTOTAL=TTOTAL+DENOM
        IF(DENOM.GT.0.1) THEN
	      WRITE(UNIT1,9211) KS,STANAME(K),STASUM4(K,1),STASUM4(K,5),
...
C
      DO M=1,6
      WRITE(UNIT1,9712) NAME(FRMODE(M)),NAME(TOMODE(M))
...
C
     ...
     *             STASUM6(K,5,M)
        ...
        TTBIK=TTBIK+STASUM6(K,5,M)
	      ...
	      TTOTAL=TTOTAL+DENOM
        IF(DENOM.GT.0.1) THEN
	      WRITE(UNIT1,9211) KS,STANAME(K),STASUM6(K,1,M),STASUM6(K,5,M),
     *               (STASUM6(K,L,M),L=2,4),DENOM
      ...
      TTOTAL=0.0
      DENOM=0.0
      WRITE(UNIT1,9713) NAME(FRMODE(M)),NAME(TOMODE(M))
...
C....LINK FREQUENCY SUMMARY
      ...
      WRITE(121,8856) 
 8856 FORMAT('LINKS,URBAN_RAIL,BLENDED,TOTAL')
      DO K=1,15
      TTRAN=SELTRIPS(K,1)+SELTRIPS(K,2)
      PNRTRP(1)=PNRTRP(1)+SELTRIPS(K,1)
      PNRTRP(2)=PNRTRP(2)+SELTRIPS(K,2)
      PNRTRP(3)=PNRTRP(3)+TTRAN
      WRITE(121,8857) K,(SELTRIPS(K,K1),K1=1,2),TTRAN
 8857 FORMAT(I2,3(',',F8.2))
      END DO
      WRITE(121,8860) (PNRTRP(K),K=1,3)
 8860 FORMAT('TOTAL',3(',',F8.2))
C.....DISTRICT SUMMARY
      OPEN(123,FILE=SLINKDIST,STATUS='UNKNOWN',FORM='FORMATTED')
      DO K1=1,MAXDIST
      DO K2=1,MAXDIST
      SELDIST(K1,K2,3)=SELDIST(K1,K2,1)+SELDIST(K1,K2,2)
      END DO
      END DO
      DO C=1,3
      DO K=1,MAXDIST
 ...
 8858 FORMAT(A10,/,'DISTRICT',21(',',A20))
      DO K=1,MAXDIST
