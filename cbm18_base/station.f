...
C...BRT
        ...
        LUNRVAL=LUNRELM(DC,15)
        NUNRVAL=NUNRELM(DC,15)
        LCRDVAL=LCROWDM(DC,15)
        NCAPVAL=NCAPACM(DC,15)
        WAIT1A=AMIN1(WAIT1(DC),WAITLT)
        WAIT1B=DIM(WAIT1(DC),WAITLT)
        STAUTL=COEFF(1)*INVEH(DC) + COEFF(3)*WAIT1A +
     *       COEFF(4)*WAIT2(DC) + COEFF(5)*TRANSF(DC)+
...
C
     ...
     *       0.04*FARE(DC)  + 2.0*WALKTFR(DC) + LUNRVAL + 2.0*NUNRVAL +
     *       LCRDVAL + 2.0*NCAPVAL
