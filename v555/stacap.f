...
C
       IF(stasum(k,15).gt.0.0) then
         IF(HBW) THEN
         OBS=(STADATA(K,4)*2.0)/STASUM(K,15)
...
C
 ...
 200  CONTINUE
C
C  SUMMARIZE COMMUTER AND URBAN RAIL ATTRIBUTES
C
      IF(CALIB) THEN
      WRITE(26,109) ((WLKPER(K2,K),K2=1,20),K=1,8)
  109 FORMAT(//1X,'WALK ACCESS TO STATION DISTANCE SUMMARIES'/
     *         1X,'-----------------------------------------'//
     *       1X,'              COMMUTER RAIL'/
     *       1X,'WALK #1     ',20F8.0/
     *       1X,'PERSON WALK ',20F8.0/
     *       1X,'WALK #2     ',20F8.0/
     *       1X,'PERSON WALK ',20F8.0//
     *       1X,'              URBAN RAIL'/
     *       1X,'WALK #1     ',20F8.0/
     ...
     *       1X,'WALK #2     ',20F8.0/
     *       1X,'PERSON WALK ',20F8.0/)
      WRITE(26,201) ((ZNEDIST(K,K1),K1=1,20),K=1,8)
  201 FORMAT(//1X,'ZONE-TO-ZONE DISTANCE SUMMARIES'/
     *         1X,'-------------------------------'//
     *       1X,'               COMMUTER RAIL'/
     *       1X,'WALK ACCESS',20(F8.1)/
     *       1X,' BUS ACCESS',20(F8.1)/
     *       1X,' P&R ACCESS',20(F8.1)/
     *       1X,' K&R ACCESS',20(F8.1)//
     *       1X,'               URBAN RAIL'/
     *       1X,'WALK ACCESS',20(F8.1)/
     *       1X,' BUS ACCESS',20(F8.1)/
     *       1X,' P&R ACCESS',20(F8.1)/
     *       1X,' K&R ACCESS',20(F8.1)/)
      WRITE(26,202) ((STADIST(K,K1),K1=1,50),K=1,8)
  202 FORMAT(//1X,'STATION-TO-STATION DISTANCE SUMMARIES'/
     *         1X,'-------------------------------------'//
     *       1X,'               COMMUTER RAIL'/
     *       1X,'WALK ACCESS',50(F8.1)/
     *       1X,' BUS ACCESS',50(F8.1)/
     *       1X,' P&R ACCESS',50(F8.1)/
     *       1X,' K&R ACCESS',50(F8.1)//
     *       1X,'               URBAN RAIL'/
     *       1X,'WALK ACCESS',50(F8.1)/
     *       1X,' BUS ACCESS',50(F8.1)/
     *       1X,' P&R ACCESS',50(F8.1)/
