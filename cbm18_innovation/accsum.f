...
C
      ...
      REAL*8    TRIPS(18),XTESUM(42,6)
      REAL*8    XDISTRN(21,21,36)
      REAL*8    XTRIPS(86,MAX_ZONES)
...
C...BIKE ACCESS STATIONS
     ...
     *     TRIPS(ISTA+12)
      ...
      END IF
      END DO    
C...UBER ACCESS STATIONS
      DO ISTA=1,4
      IF(TRIPS(ISTA+14).LE.0.0) CYCLE
      IF(CHKIND(IMODE,ISTA)) THEN
      SC2=KDSTA((IMODE+5),ISTA)-MAX_IZONES
      IF(IMODE.EQ.1) THEN
       IF(STANUM(SC2).EQ.2) THEN                                     !CR --> Urban Rail
       XTESUM(41,C)=XTESUM(41,C)+TRIPS(ISTA+14)
       ELSE 
       XTESUM(42,C)=XTESUM(42,C)+TRIPS(ISTA+14)                       !CR --> BRT 
       XTRIPS(72,JZ)=XTRIPS(72,JZ)+TRIPS(ISTA+14)                      
       END IF      
      CYCLE
      END IF
      IF(STANUM(SC2).EQ.1) THEN
      IF(IMODE.EQ.2) XTESUM(37,C)=XTESUM(37,C)+TRIPS(ISTA+14)
      IF(IMODE.EQ.5) XTESUM(38,C)=XTESUM(38,C)+TRIPS(ISTA+14)
      IF(IMODE.EQ.5) XTRIPS(71,JZ)=XTRIPS(71,JZ)+TRIPS(ISTA+14)
      ELSE
       IF(STANUM(SC2).EQ.2) THEN                                     !BRT --> Urban Rail
       XTESUM(39,C)=XTESUM(39,C)+TRIPS(ISTA+14)
       XTRIPS(73,JZ)=XTRIPS(73,JZ)+TRIPS(ISTA+14)
       ELSE
       XTESUM(40,C)=XTESUM(40,C)+TRIPS(ISTA+14)                       !Urban Rail --> BRT
       XTRIPS(74,JZ)=XTRIPS(74,JZ)+TRIPS(ISTA+14)
       END IF
      END IF
      END IF
      END DO   
