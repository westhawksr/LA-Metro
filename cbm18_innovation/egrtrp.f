...
C
      ...
      REAL*4    RALTNC(MAX_STATIONS,MAX_IZONES)
...
C...UBER ACCESS STATIONS
      ...
      SC2=KDSTA((IMODE+5),ISTA)-MAX_IZONES      
      ALLSTAZ(SC2,JZ)=ALLSTAZ(SC2,JZ)+
     *   (SNGL(TRIPS(ISTA+14))*STAEGR(2,SC2,JZ))
      ALLWLK(SC2,JZ)=ALLWLK(SC2,JZ)+
     *   (SNGL(TRIPS(ISTA+14))*STAEGR(1,SC2,JZ))
      ALLTNC(SC2,JZ)=ALLTNC(SC2,JZ)+
     *   (SNGL(TRIPS(ISTA+14))*STAEGR(4,SC2,JZ))
      ELSE
      SC=ASTA(IMODE,(ISTA+8))-MAX_IZONES
      RALSTAZ(SC,JZ)=RALSTAZ(SC,JZ)+
     *   (SNGL(TRIPS(ISTA+14))*STAEGR(2,SC,JZ))
      RALWLK(SC,JZ)=RALWLK(SC,JZ)+
     *   (SNGL(TRIPS(ISTA+14))*STAEGR(1,SC,JZ))
      RALTNC(SC,JZ)=RALTNC(SC,JZ)+
     *   (SNGL(TRIPS(ISTA+14))*STAEGR(4,SC,JZ))
      END IF
      END DO         
      RETURN
      END
