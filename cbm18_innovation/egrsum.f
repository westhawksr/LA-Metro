...
C
      ...
      REAL*8    TRIPS(18),STASUM2(MAX_STATIONS,5)
      REAL*8    STASUM4(MAX_STATIONS,6),RTEMP,TTEMP
      REAL*8    STASUM5(MAX_STATIONS,5),STAVOL(5,5)
...
C...WALK ACCESS STATIONS
      ...
      TTEMP=TRIPS(ISTA)*STAEGR(4,SC2,JZ)
      STASUM4(DC,1)=STASUM4(DC,1)+TRIPS(ISTA)
      STASUM5(SC2,1)=STASUM5(SC2,1)+RTEMP
      STASUM5(SC2,5)=STASUM5(SC2,5)+TTEMP
      STASUM5(SC2,2)=STASUM5(SC2,2)+TRIPS(ISTA)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA)
      CALL XFERSUM(2,1,DC,SC2,TRIPS(ISTA),RTEMP,TTEMP)
      END IF
      ELSE
      DC=OSTA(IMODE,ISTA)-MAX_IZONES
      SC=ASTA(IMODE,ISTA)-MAX_IZONES
      ...
      TTEMP=TRIPS(ISTA)*STAEGR(4,SC,JZ)
      STASUM2(SC,1)=STASUM2(SC,1)+RTEMP
      STASUM2(SC,5)=STASUM2(SC,5)+TTEMP
      STASUM2(SC,2)=STASUM2(SC,2)+TRIPS(ISTA)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN 
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA)
      CALL XFERSUM(1,1,DC,SC,TRIPS(ISTA),RTEMP,TTEMP)
      END IF
      END IF
      END DO     
C...BUS ACCESS STATIONS
      ...
      TTEMP=TRIPS(ISTA+2)*STAEGR(4,SC2,JZ)
      STASUM4(DC,2)=STASUM4(DC,2)+TRIPS(ISTA+2)
      STASUM5(SC2,1)=STASUM5(SC2,1)+RTEMP
      STASUM5(SC2,5)=STASUM5(SC2,5)+TTEMP
      STASUM5(SC2,2)=STASUM5(SC2,2)+TRIPS(ISTA+2)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+2)
      CALL XFERSUM(2,2,DC,SC2,TRIPS(ISTA+2),RTEMP,TTEMP)
      END IF
      ELSE
      DC=OSTA(IMODE,(ISTA+2))-MAX_IZONES
      SC=ASTA(IMODE,(ISTA+2))-MAX_IZONES
      ...
      TTEMP=TRIPS(ISTA+2)*STAEGR(4,SC,JZ)
      STASUM2(SC,1)=STASUM2(SC,1)+RTEMP
      STASUM2(SC,5)=STASUM2(SC,5)+TTEMP
      STASUM2(SC,2)=STASUM2(SC,2)+TRIPS(ISTA+2)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN 
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+2)
      CALL XFERSUM(1,2,DC,SC,TRIPS(ISTA+2),RTEMP,TTEMP)
      END IF
      END IF
      END DO      
C...PNR ACCESS STATIONS
      ...
      TTEMP=TRIPS(ISTA+4)*STAEGR(4,SC2,JZ)
      STASUM4(DC,3)=STASUM4(DC,3)+TRIPS(ISTA+4)
      STASUM5(SC2,1)=STASUM5(SC2,1)+RTEMP
      STASUM5(SC2,5)=STASUM5(SC2,5)+TTEMP
      STASUM5(SC2,2)=STASUM5(SC2,2)+TRIPS(ISTA+4)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+4)
      CALL XFERSUM(2,3,DC,SC2,TRIPS(ISTA+4),RTEMP,TTEMP)
      END IF
      ELSE
      DC=OSTA(IMODE,(ISTA+4))-MAX_IZONES
      SC=ASTA(IMODE,(ISTA+4))-MAX_IZONES
      ...
      TTEMP=TRIPS(ISTA+4)*STAEGR(4,SC,JZ)
      STASUM2(SC,1)=STASUM2(SC,1)+RTEMP
      STASUM2(SC,5)=STASUM2(SC,5)+TTEMP
      STASUM2(SC,2)=STASUM2(SC,2)+TRIPS(ISTA+4)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+4)
      CALL XFERSUM(1,3,DC,SC,TRIPS(ISTA+4),RTEMP,TTEMP)
      END IF
      END IF
      END DO   
C...KNR ACCESS STATIONS
      ...
      TTEMP=TRIPS(ISTA+8)*STAEGR(4,SC2,JZ)
      STASUM4(DC,4)=STASUM4(DC,4)+TRIPS(ISTA+8)
      STASUM5(SC2,1)=STASUM5(SC2,1)+RTEMP
      STASUM5(SC2,5)=STASUM5(SC2,5)+TTEMP
      STASUM5(SC2,2)=STASUM5(SC2,2)+TRIPS(ISTA+8)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+8)
      CALL XFERSUM(2,4,DC,SC2,TRIPS(ISTA+8),RTEMP,TTEMP)
      END IF
      ELSE
      DC=OSTA(IMODE,(ISTA+8))-MAX_IZONES
      SC=ASTA(IMODE,(ISTA+8))-MAX_IZONES
      ...
      TTEMP=TRIPS(ISTA+8)*STAEGR(4,SC,JZ)
      STASUM2(SC,1)=STASUM2(SC,1)+RTEMP
      STASUM2(SC,5)=STASUM2(SC,5)+TTEMP
      STASUM2(SC,2)=STASUM2(SC,2)+TRIPS(ISTA+8)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+8)
      CALL XFERSUM(1,4,DC,SC,TRIPS(ISTA+8),RTEMP,TTEMP)
      END IF
      END IF
      END DO       
C...BIKE ACCESS STATIONS
      ...
      TTEMP=TRIPS(ISTA+12)*STAEGR(4,SC2,JZ)
      STASUM4(DC,5)=STASUM4(DC,5)+TRIPS(ISTA+12)
      STASUM5(SC2,1)=STASUM5(SC2,1)+RTEMP
      STASUM5(SC2,5)=STASUM5(SC2,5)+TTEMP
      STASUM5(SC2,2)=STASUM5(SC2,2)+TRIPS(ISTA+12)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+12)
      CALL XFERSUM(2,5,DC,SC2,TRIPS(ISTA+12),RTEMP,TTEMP)
      END IF
      ELSE
      DC=OSTA(IMODE,(ISTA+12))-MAX_IZONES
      SC=ASTA(IMODE,(ISTA+12))-MAX_IZONES
      ...
      TTEMP=TRIPS(ISTA+12)*STAEGR(4,SC,JZ)
      STASUM2(SC,1)=STASUM2(SC,1)+RTEMP
      STASUM2(SC,5)=STASUM2(SC,5)+TTEMP
      STASUM2(SC,2)=STASUM2(SC,2)+TRIPS(ISTA+12)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN 
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+12)
      CALL XFERSUM(1,5,DC,SC,TRIPS(ISTA+12),RTEMP,TTEMP)
      END IF
      END IF
      END DO  
C...UBER ACCESS STATIONS
      ...
      SC2=KDSTA((IMODE+5),ISTA)-MAX_IZONES
      DMODE=STANUM(SC2)
      RTEMP=TRIPS(ISTA+14)*STAEGR(1,SC2,JZ)
      TTEMP=TRIPS(ISTA+14)*STAEGR(4,SC2,JZ)
      STASUM4(DC,6)=STASUM4(DC,6)+TRIPS(ISTA+14)
      STASUM5(SC2,1)=STASUM5(SC2,1)+RTEMP
      STASUM5(SC2,5)=STASUM5(SC2,5)+TTEMP
      STASUM5(SC2,2)=STASUM5(SC2,2)+TRIPS(ISTA+14)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+14)
      CALL XFERSUM(2,6,DC,SC2,TRIPS(ISTA+14),RTEMP,TTEMP)
      END IF
      ELSE
      DC=OSTA(IMODE,(ISTA+8))-MAX_IZONES
      SC=ASTA(IMODE,(ISTA+8))-MAX_IZONES
      DMODE=STANUM(SC)
      RTEMP=TRIPS(ISTA+14)*STAEGR(1,SC,JZ)
      TTEMP=TRIPS(ISTA+14)*STAEGR(4,SC,JZ)
      STASUM2(SC,1)=STASUM2(SC,1)+RTEMP
      STASUM2(SC,5)=STASUM2(SC,5)+TTEMP
      STASUM2(SC,2)=STASUM2(SC,2)+TRIPS(ISTA+14)-RTEMP-TTEMP
      IF(DMODE.GT.0) THEN
      STAVOL(IMODE,DMODE)=STAVOL(IMODE,DMODE)+TRIPS(ISTA+14)
      CALL XFERSUM(1,6,DC,SC,TRIPS(ISTA+14),RTEMP,TTEMP)
      END IF
      END IF
      END DO       
      END IF
